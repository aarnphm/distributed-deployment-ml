...
onnx-d-r:
	docker run --gpus all -p 50053:5000 $(DEVICE_ARGS) bento-onnx-gpu:latest
...
torch-d-r:
	docker run --gpus all -p 50052:5000 $(DEVICE_ARGS) bento-torch-gpu:latest
...
tf-d-r:
	docker run --gpus all -p 50051:5000 $(DEVICE_ARGS) bento-tf-gpu:latest